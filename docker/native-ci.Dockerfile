...
RUN apt-get update \
    && apt-get install -y --no-install-recommends cmake ninja-build gcc g++ git libssl-dev libprotobuf-dev libabsl-dev libcurl4-openssl-dev protobuf-compiler pkg-config \
