...
RUN apt-get update \
    && apt-get install -y --no-install-recommends cmake ninja-build gcc g++ git iproute2 python3 libssl-dev libprotobuf-dev libabsl-dev libcurl4-openssl-dev protobuf-compiler pkg-config \
...
RUN git clone --filter=blob:none https://github.com/ValveSoftware/GameNetworkingSockets.git native/GameNetworkingSockets \
    ...
    && cmake -S native/GameNetworkingSockets -B native/build -G Ninja -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTS=ON -DBUILD_EXAMPLES=OFF \
...
RUN ctest --test-dir /src/native/build --output-on-failure -R '^connection_quick$'
