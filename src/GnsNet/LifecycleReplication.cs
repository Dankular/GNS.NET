...
public sealed class LifecycleReplicationScheduler<TClientId> where TClientId : notnull
{
    ...
    { this.registry = registry ?? throw new ArgumentNullException(nameof(registry)); this.opcode = opcode; this.encode = encode ?? throw new ArgumentNullException(nameof(encode)); this.registry.Changed += this.OnChanged; }
